...
class _ChatListScreenState extends State<ChatListScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () async {
              final messagingProvider = Provider.of<MessagingProvider>(context, listen: false);
              // Use a hardcoded test user ID (change as needed)
      ...
      body: Consumer<MessagingProvider>(
        builder: (context, messagingProvider, child) {
          ...
          if (messagingProvider.error != null && messagingProvider.error!.isNotEmpty) {
            return Center(
              child: Column(
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemCount: messagingProvider.chats
                  .where((chat) => messagingProvider.getOtherParticipantName(chat.id) != 'Unknown User')
                  .length,
              itemBuilder: (context, index) {
                final filteredChats = messagingProvider.chats
                    .where((chat) => messagingProvider.getOtherParticipantName(chat.id) != 'Unknown User')
                    .toList();
  ...
  Widget _buildChatTile(ChatModel chat, MessagingProvider messagingProvider) {
    ...
    final unreadCount = chat.unreadCounts[messagingProvider.currentUserId] ?? 0;
            ...
            content: SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    leading: CircleAvatar(
                      child: Text(user['name'] != null && user['name'].isNotEmpty ? user['name'][0].toUpperCase() : 'U'),
                    ),
                    ...
                    onTap: () async {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Started chat with ${user['name'] ?? 'Unknown'}')),
                        );
                      }
                    },
                  );
