...
class NavigationService extends ChangeNotifier {
  ...
  // Set current navigation index and update Firestore
  void setCurrentIndex(int index) async {
    // Defensive: Only allow valid tab indices (0-5)
    final int safeIndex = (index is int && index >= 0 && index < 6) ? index : 0;
    ...
    if (user != null) {
      try {
        await _firestore
            .collection('users')
            .doc(user.uid)
            .update({'lastTabIndex': safeIndex});
      } catch (e) {
        debugPrint('Error saving lastTabIndex: $e');
      }
    }
  }

  // Fetch last tab index from Firestore
  Future<int> fetchLastTabIndex() async {
    final user = _auth.currentUser;
    if (user != null) {
      try {
        ...
        if (doc.exists &&
            doc.data() != null &&
            doc.data()!.containsKey('lastTabIndex')) {
          final dynamic idx = doc['lastTabIndex'];
          if (idx is int && idx >= 0 && idx < 6) {
            return idx;
          }
        }
      } catch (e) {
        debugPrint('Error fetching lastTabIndex: $e');
      }
    }
