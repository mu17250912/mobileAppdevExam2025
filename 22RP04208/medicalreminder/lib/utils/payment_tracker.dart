...
class PaymentTracker {
  ...
  Future<void> processPayment({
    ...
    try {
      ...
      // Poll for real payment confirmation
      bool isPaid = false;
      int attempts = 0;
      while (!isPaid && attempts < 12) {
        ...
        if (verifyResult['status'] == 200) {
          final vresp = verifyResult['response'];
          if ((vresp is Map && (vresp['paid'] == true || vresp['success'] == true || vresp['status'] == 'success')) ||
              (vresp is String && (vresp.toLowerCase().contains('success') || vresp.toLowerCase().contains('paid')))) {
            isPaid = true;
            break;
          }
        } else {
          // Network or API error, do NOT upgrade
          print('[PaymentTracker] Network or API error during verification, not upgrading.');
        }
        attempts++;
      }
      if (isPaid) {
        print('[PaymentTracker] Payment verified successfully, upgrading to premium');
        await updatePaymentStatus(
          paymentId: paymentId,
          status: 'completed',
          transactionId: 'verified_payment',
        );
      } else {
        print('[PaymentTracker] Payment not confirmed or network error, marking as timeout/failed');
        await updatePaymentStatus(
          paymentId: paymentId,
          status: 'timeout',
          transactionId: 'not_confirmed',
          errorMessage: 'Payment not confirmed in time or network error',
        );
      }
    } catch (e) {
